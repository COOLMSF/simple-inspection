...
def DBA_CHECKOUT(wb):
    ...
    info_title_list = ["节点名称", "ip", "型号别名", "型号",
                       "是否可扩容", "总实例数", "可分配的实例数", "总模板数", "模板类型", "授权天数", "可分配模板数", "该类型模板总数", "过期模板数"]
    ...
    for name, ip in node_dic.items():
        ...
        url = f'https://{ip}/cas/templateStatus/type/new'
        ...
        for data_dic in data_list:
            ...
            for key, value in data_dic.items():
                if key in ["model", "model_alias", "is_expand", "instanceNum", "useableInstanceNum", "templateNum"]:
                ...
                if key == "templateCount":
                    for template_key, template_value in value.items():
                        column = 9
                        for template_value1 in template_value.values():
                            sheet.cell(row=row, column=column).value = template_value1
                            column += 1
                        row += 1
